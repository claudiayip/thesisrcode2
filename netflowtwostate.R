...
MMPP <- function(N, aL, bL,sig){
  ...
  A0 <- matrix(1/2,2,2)
  aLS <- aL ; bLT <- bL
  abcdZ1 <- matrix(1680/4,1,2); abcdZ2 <- matrix(1680/4,1,2)
  abcdZ3 <- matrix(1680/4,1,2); abcdZ4 <- matrix(1680/4,1,2)
  alphadsj <- alphad
  alphansij <- alphan
  ## loop ##
  ...
  for (kk in 1:150){
    ...
    zt <- as.numeric(0)
    zt[1680] <- sample(c(0,1),1, prob= gamma[,1680])
    for (tt in 1679:1){
  ...
  List <- list("lv"= lambda0v, "zv"=zv, "z"=zt, "ltv" = lambdatv, "NO" = NO, "NE" = Ne, "a0" = A0)
  return(List)
}



bb <- MMPP(N, aL= sum(N), bL= 1680 ,sig = sigma)



par(mfrow = c(1,1))
plot(N,type = 'l', xlab = "Time bin",ylim = c(0,max(N)*2), ylab = "")
par(new=TRUE)
plot(a$z, type = 'l', col = "red", ylab = "", xlab = "",
     ylim = c(-3,2), yaxt = "n")
par(new = TRUE)
...
par(mfrow = c(1,1))
plot(N[c(1345:1680)], pch = ".", ylim = c(0,max(N)*2), xlab = "Time bins", ylab = '') #  main = "C1085 and C553",
lines(N[c(1345:1680)])
par(new = TRUE)
plot(a$z[c(1345:1680)], pch = ".", col = "red", ylab = "", xlab = "",
     ylim = c(-2,2), yaxt = "n")
lines(a$z[c(1345:1680)], col = "red")
par(new = TRUE)
plot(a$ltv[c(1345:1680)], pch = '.', col = 'green', ylab = '', ylim = c(0,max(N)*2),
     xlab = '')
lines(a$ltv[c(1345:1680)], col = "green")
