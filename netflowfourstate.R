...
MMPP4 <- function(N, aL, bL,sig){
  q95 <- as.numeric(quantile(N, probs = 0.9))
  aE = q95
  bE = 2
  ...
  for (kk in 1:150) {
    ...
    pznznm13 <- A0[,4]
    # Forward recursion
    alphazn1v<- matrix(0,4,1680)
    # Set initial value of forward recursion 
    initialalpha <- c(1/4,1/4,1/4,1/4) * 
    ...
    pztzt1 <- array(0,c(4,4,1680))
    pztzt1v <- array(0,c(4,4,1680))
    for (t in 1679:1){
      for (p in 1:4){
        for (q in 1:4){
          pztzt1v[p,q,t] <- alphazn1v[p,t]  * Pntzt(t+1,(q-1)) *
    ...
    zt <- as.numeric(0)
    zt[1680] <- sample(c(0,1,2,3),1, prob = gamma[,1680])
    for (tt in 1679:1){
...
dd <- MMPP4(N, aL = sum(N)/2, bL = 1680,sig = sigma)
